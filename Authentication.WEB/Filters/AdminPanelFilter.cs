...
{
    public class AdminPanelFilter : ActionFilterAttribute, IActionFilter
    {
        void IActionFilter.OnActionExecuted(ActionExecutedContext filterContext)
        {
            using (InsuredTravelingEntity baza = new InsuredTravelingEntity())
            {
                ActionLog1 log = new ActionLog1()
                {
                    ...
                    action = string.Concat(filterContext.ActionDescriptor.ActionName, " (Logged By: Admin Panel Filter)"),
